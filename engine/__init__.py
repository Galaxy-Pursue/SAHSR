...
from data.sampler import DynamicSampler
...
def get_trainer(dataset, model, optimizer, lr_scheduler=None, logger=None, writer=None, non_blocking=False, log_period=10,
    ...
    # trainer
    trainer = create_train_engine(model, optimizer, non_blocking, p=p_size)
    ...
    def train_completed(engine):
        ...
        if dataset == 'sysu':
            perm = sio.loadmat(os.path.join(dataset_cfg.sysu.data_root, 'exp', 'rand_perm_cam.mat'))[
                'rand_perm_cam']
            logging.info('sysu evaluation')
            eval_sysu(q_feats, q_ids, q_cams, g_feats, g_ids, g_cams, g_img_paths, perm, mode='all', num_shots=1, rerank=True)
            eval_sysu(q_feats, q_ids, q_cams, g_feats, g_ids, g_cams, g_img_paths, perm, mode='all', num_shots=10, rerank=True)
            eval_sysu(q_feats, q_ids, q_cams, g_feats, g_ids, g_cams, g_img_paths, perm, mode='indoor', num_shots=1, rerank=True)
            eval_sysu(q_feats, q_ids, q_cams, g_feats, g_ids, g_cams, g_img_paths, perm, mode='indoor', num_shots=10, rerank=True)
        elif dataset == 'regdb':
            logging.info('regdb evaluation')
            logging.info('infrared to visible')
            eval_regdb(q_feats, q_ids, q_cams, g_feats, g_ids, g_cams, g_img_paths, rerank=True)
            logging.info('visible to infrared')
            eval_regdb(g_feats, g_ids, g_cams, q_feats, q_ids, q_cams, q_img_paths, rerank=True)
        else:
            raise "Not surpported dataset"
    ...
    def epoch_completed_callback(engine):
        ...
        if inf_evaluator and vis_evaluator and epoch % eval_interval == 0 and epoch > start_eval:
            ...
            if dataset == 'sysu':
                perm = sio.loadmat(os.path.join(dataset_cfg.sysu.data_root, 'exp', 'rand_perm_cam.mat'))[
                    'rand_perm_cam']
                mAP, r1, r5, r10, r20 = eval_sysu(q_feats, q_ids, q_cams, g_feats, g_ids, g_cams, g_img_paths, perm, mode='all', num_shots=1,
                           rerank=True)
            elif dataset == 'regdb':
                logging.info('infrared to visible')
                mAP, r1, r5, _, _ = eval_regdb(q_feats, q_ids, q_cams, g_feats, g_ids, g_cams, g_img_paths, rerank=True)
                logging.info('visible to infrared')
                mAP, r1_, r5, _, _ = eval_regdb(g_feats, g_ids, g_cams, q_feats, q_ids, q_cams, q_img_paths, rerank=True)
                r1 = (r1 + r1_) / 2
    ...
    def iteration_complete_callback(engine):
        ...
        if epoch >= extra_start_epoch:
            ...
            # find top_k idx for additional training
